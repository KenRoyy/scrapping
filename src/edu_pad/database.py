...
class DataBase:
    def __init__(self):
        self.rutadb = "src/edu_pad/static/db/google_analisis.db"
    ...
    def guardar_df(self,df=pd.DataFrame()):
        ...
        try:
            ...
            df.to_sql("google_analisis",conn,if_exists='replace',index=False)
            print("*******************************************************************")
    ...
    def obtener_datos(self,nombre_tabla="google_analisis"):
        try:
            conn = sqlite3.connect(self.rutadb)
